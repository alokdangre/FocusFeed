...
The intended future contract keeps assessment labels independent of filtering mode so current policy can reinterpret a cached assessment. Today both provider requests include mode, so cache schema v2 conservatively includes it as well. A mode change therefore misses the cache. Establish and version a mode-independent semantic request before enabling reuse across modes.
...
Checkpoint A1's core router/cache flow passed Alok's persisted, empty-store, and cold browser reports. The YouTube feed and replay share the deterministic rule evaluator and policy. Channel controls use exact normalized identity, missing duration is not proof of a Short, cache schema v2 validates identity/labels/timestamps, and cooperating writes are serialized. The live YouTube allow/format selector check is deferred; semantic decisions remain disconnected from live hiding.

Checkpoint B1 adds a reusable bounded scheduler plus Lifecycle Replay. It covers a 100-card burst, priority overflow, duplicate subscribers, scroll-away cancellation, goal changes, pause/resume, late responses, timeout, provider error, missing results, stage timings, visible expectations, and JSON export. It uses a fake provider and does not measure local AI, Bedrock, or DOM latency. Follow handoff B1 in [WORKFLOW_EVALUATION_PLAN.md](WORKFLOW_EVALUATION_PLAN.md).
...
6. **Qualified through deterministic B1:** shared rule/cache routing, bounded priority scheduling, batching, in-flight deduplication, subscriber cancellation, stale-result protection, pause/resume, failure validation, stage timing, visible fixtures, and report export. Alok's lifecycle report passed all five scenarios. See [WORKFLOW_AND_LATENCY_PLAN.md](WORKFLOW_AND_LATENCY_PLAN.md).
7. **C1 qualified:** the real Chrome built-in provider passed the fixed 12-video, three-call scheduler smoke with full coverage, zero decision errors, zero Focus false hides, and no output-integrity fault. Provider p95 was 59.3s, so the result does not qualify immediate live-feed decisions.
8. **YouTube integration implemented in 0.6.0; browser report pending:** real Home cards use rules, a separate live assessment cache, and bounded residual inference. A session tab starts in preview and offers opt-in reversible AI hides. Admission starts at 12 videos and at most three provider calls; only explicit Load more expands it. Home continuation gating prevents automatic replenishment. See [YOUTUBE_LIVE_TEST.md](YOUTUBE_LIVE_TEST.md) for the manual handoff.
9. Benchmark compact local classification output and base-session cloning as separate experiments against C1 and the small shadow run. Then conduct a 30-50 recommendation shadow review before any reversible live semantic pilot.
10. Re-enable and benchmark Bedrock on the same workflow and inputs as soon as AWS removes the account restriction. Local timings do not establish Bedrock latency.
